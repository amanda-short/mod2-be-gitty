...
const { exchangeCodeForToken } = require('../services/ghservices.js');

const { getGithubProfile } = require('../services/ghservices.js');
...
module.exports = Router()
  ...
  .get('/callback', async (req, res, next) => {
    try {
      ...
      const user = await GithubUser.findBylogin(login);
      if (!user) {
        GithubUser.insert({
          login,
          email,
